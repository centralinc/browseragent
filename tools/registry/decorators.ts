...
export function capability(
  ...
  return function (
    ...
    const metadata =
      Reflect.getMetadata("capability:metadata", target, propertyKey) || {};
...
export function registerCapabilities(
  ...
  methodNames.forEach((methodName) => {
    ...
    // Get metadata
    const metadata =
      Reflect.getMetadata("capability:metadata", prototype, methodName) || {};
...
export function withCapabilities<T extends { new (...args: any[]): object }>(
  constructor: T,
): T {
  return class extends constructor {
    ...
    constructor(...args: any[]) {
      ...
      const toolName =
        instance.name || constructor.name.toLowerCase().replace(/tool$/, "");
