 ...
 */
export const PLAYWRIGHT_CAPABILITIES: Map<string, PlaywrightCapabilityDef> =
  new Map();
...
The tool will search for the text and extract the associated URL`,
  ...
  handler: async (page: Page, args: string[]): Promise<ToolResult> => {
    ...
    if ((await textElement.count()) > 0) {
      ...
      if (!url) {
        const parentAnchor = await textElement
    ...
    if (!url) {
      const anchorWithText = await page
        .locator(`a:has-text("${selector}")`)
        .first();
 ...
 */
export function registerPlaywrightCapability(
  capability: PlaywrightCapabilityDef,
): void {
 ...
 */
export function generatePlaywrightDocs(
  capabilities?: PlaywrightCapabilityDef[],
): string {
