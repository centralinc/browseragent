...
async function scrollToTextExample(): Promise<void> {
  ...
  try {
    ...
    console.log("===============================\n");
    console.log(
      "Testing agent's ability to choose optimal scrolling strategies automatically\n",
    );
    ...
    console.log("\n✅ Key Takeaways:");
    console.log(
      "• Agent intelligently chooses instant text navigation when appropriate",
    );
    console.log("• Falls back gracefully to regular scrolling when needed");
    console.log(
      "• No manual tool selection required - agent decides based on context",
    );
    console.log(
      "• Optimized for finding specific items in dropdowns and lists",
    );
  } catch (error) {
