...
async function signalsExample(): Promise<void> {
  ...
  try {
    ...
    agent.controller.on("onCancel", ({ at, step, reason }) => {
      console.log(
        `🔴 Agent cancelled at step ${step} (${at.toISOString()})${reason ? `: ${reason}` : ""}`,
      );
    });

    agent.controller.on("onError", ({ at, step, error }) => {
      console.log(
        `❌ Agent error at step ${step} (${at.toISOString()}):`,
        error,
      );
    });
...
async function manualControlExample(): Promise<void> {
  ...
  try {
    ...
    console.log(
      "This example shows how you can control the agent from external code",
    );
...
export { signalsExample, manualControlExample, structuredDataWithSignals };
